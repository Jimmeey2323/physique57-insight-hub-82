...
export const ImprovedLeadSourcePerformanceTable: React.FC<LeadSourcePerformanceTableProps> = ({
  ...
  const columns = [
        ...
        <TooltipProvider>
          <Tooltip>
            <TooltipTrigger asChild>
              <div className="flex items-center gap-2 cursor-pointer" onClick={() => toggleRowExpansion(value)}>
                <div className="w-2 h-2 rounded-full bg-gradient-to-r from-blue-500 to-purple-600" />
                <span className="font-medium text-slate-800">{value}</span>
                <Button
                  variant="ghost"
                  size="sm"
                  className="ml-2 opacity-60 hover:opacity-100 p-1 h-6 w-6"
                >
    ...
    },
    {
      ...
      render: (value: any) => (
        <div className="text-center">
          <div className="font-bold text-slate-900">{formatNumber(safeNumber(value))}</div>
    ...
    },
    {
      ...
      render: (value: any) => (
        <div className="text-center">
          <div className="font-bold text-slate-900">{safeToFixed(value)}%</div>
    ...
    },
    {
      ...
      render: (value: any) => (
        <div className="text-right">
          <div className="font-bold text-slate-900">{formatCurrency(safeNumber(value))}</div>
  ...
  const calculateTotals = () => {
    ...
    }), { totalLeads: 0, trialsCompleted: 0, converted: 0, totalLost: 0, totalRevenue: 0 });
  };
